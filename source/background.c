...
int background_ncdm_momenta(
  ...
  for (index_q=0; index_q<qsize; index_q++) {
    ...
    if(z>zq){
      ...
      /* integrand of the various quantities */ /*tried to modify here*/
    ...
    if(background_ncdm_distribution == _decaying_neutrinos_){
      ...
      */
      // printf("t %e\n", t);
        if (n!=NULL) *n += q2*wvec[index_q]*exp(-pba->Gamma_neutrinos*M/(epsilon*(1+z))*t);
        if (rho!=NULL) *rho += q2*epsilon*wvec[index_q]*exp(-pba->Gamma_neutrinos*M/(epsilon*(1+z))*t);
        if (p!=NULL) *p += q2*q2/3./epsilon*wvec[index_q]*exp(-pba->Gamma_neutrinos*M/(epsilon*(1+z))*t);
        if (drho_dM!=NULL) *drho_dM += q2*M/(1.+z)/(1.+z)/epsilon*wvec[index_q]*exp(-pba->Gamma_neutrinos*M/(epsilon*(1+z))*t);
        if (pseudo_p!=NULL) *pseudo_p += pow(q2/epsilon,3)/3.0*wvec[index_q]*exp(-pba->Gamma_neutrinos*M/(epsilon*(1+z))*t);
  ...
  if (n!=NULL) *n *= factor2/(1.+z)/(pba->T_cmb*pba->T_ncdm[0]*_k_B_/_h_P_/2./_PI_/_c_*_Mpc_over_m_); //one extra factor of 2pi is weird...
  // pba->factor_ncdm[k]=pba->deg_ncdm[k]*4*_PI_*pow(pba->T_cmb*pba->T_ncdm[k]*_k_B_,4)*8*_PI_*_G_
    ...
    if(pba->has_ncdm == _TRUE_ && pba->Gamma_neutrinos > 0){
      ...
      for(n_ncdm = 0; n_ncdm<pba->N_ncdm; n_ncdm++){
        if(pba->background_ncdm_distribution[n_ncdm] == _decaying_neutrinos_){
          dy[pba->index_bi_rho_dr] += y[pba->index_bi_a]*pba->Gamma_neutrinos*pvecback[pba->index_bg_n_ncdm1+n_ncdm]*pba->m_ncdm_in_eV[n_ncdm]*_eV_/_h_P_/2./_PI_/_c_*_Mpc_over_m_*_PI_/2;///_eV_/_h_P_/2./_PI_/_c_*_Mpc_over_m_ convert from eV to 1/Mpc. One extra factor of pi/2 is weird. 
